...
function formatPrice(amount: number | undefined, currency: Currency, locale: Locale) {
...
/** ====== API: Posts ====== */
async function fetchPostsByLocale(locale: Locale) {
  const url = `${STRAPI_URL}/api/posts?populate=cover&locale=${locale}&sort=publishedAt:desc`;
  const res = await fetch(url, { cache: "no-store" });
  if (!res.ok) throw new Error(`Strapi ${res.status}: ${await res.text()}`);
  const json = await res.json();
  return (json.data ?? []) as any[];
...
async function fetchPostsWithFallback(
  ...
  const primary = await fetchPostsByLocale(preferred);
  if (primary.length) return { data: primary, usedLocale: preferred };
  const alt: Locale = preferred === "ru" ? "en" : "ru";
  const secondary = await fetchPostsByLocale(alt);
  return { data: secondary, usedLocale: alt };
}
...
const DICT = {
  ru: {
    ...
    heroText:
      "Мы сопровождаем строительство вертолётов Ми-8 от выбора комплектации до ввода в эксплуатацию. Работаем с Казанским и Улан-Удэнским заводами и берём на себя все переговоры, контроль и приёмку.",
    ...
    whatText:
      "Берём на себя всё, что владелец не сможет сделать сам — от выбора комплектации до приёмки и управления.",
    whatItems: [
    ...
    whyLead:
      "Покупка Ми-8 без профессионального сопровождения = лишние расходы, задержки и риски.",
    whyStats: [
    ...
    expTitle: "Наш опыт",
    expFacts: [
      "14 воздушных судов в управлении компании",
      "Приёмка Ми-8 с Казанского и Улан-Удэнского заводов",
      "Клиенты из России, Азии и Ближнего Востока",
    ],
    casesTitle: "Кейсы",
    cases: [
    ...
    blogTitle: "Блог",
    blogLead: "Статьи, разборы и новости по Ми-8, комплектации, приёмке и эксплуатации.",
    readMore: "Читать",
    ...
    contactLead:
      "Ответим на вопросы, обсудим задачи и предложим оптимальную конфигурацию под ваши цели.",
    formName: "Имя",
    formPhone: "Телефон",
    formMsg: "Сообщение",
    formSendMail: "Отправить на почту",
    formWriteTG: "Написать в Telegram",
  ...
  en: {
    ...
    heroText:
      "We supervise Mi-8 builds end-to-end: configuration, factory control, final acceptance and entry into service.",
    ctaConsult: "Request a consultation",
    ctaCall: "Call us",
    bulletTiming: "Final acceptance in 4–5 business days vs typical 2 weeks",
    bulletMoney: "Save up to $500,000 on unnecessary options",
    bulletFull: "Full cycle: from requirements to entry into service",
    whatTitle: "Full-cycle Mi-8 acquisition support",
    whatText:
      "We cover everything — from configuration to factory acceptance and airline management.",
    whatItems: [
    ...
    whyLead:
      "Buying a Mi-8 without professional support means extra costs, delays and risks.",
    whyStats: [
    ...
    expTitle: "Experience",
    expFacts: [
      "14 aircraft under management",
      "Mi-8 acceptances from Kazan and Ulan-Ude plants",
      "Clients across Russia, Asia and the Middle East",
    ],
    ...
    blogTitle: "Blog",
    blogLead:
      "Articles, explainers and news about Mi-8 specifications, acceptance and operations.",
    readMore: "Read",
    marketTitle: "Marketplace — pre-owned Mi-8",
    marketLead:
      "Selection, history checks, technical inspection and deal support end-to-end.",
    marketCTA: "Request details",
    contactTitle: "Get in touch",
    contactLead:
      "We’ll answer questions, discuss your mission and propose the optimal configuration.",
    formName: "Name",
    formPhone: "Phone",
    formMsg: "Message",
    formSendMail: "Send by email",
    formWriteTG: "Message on Telegram",
      ...
      <Container className="flex items-center justify-between h-16">
        ...
        <nav className="hidden md:flex items-center gap-6">
          {[
            ...
            <a key={n.id} href={`#${n.id}`} className="text-sm text-gray-700 hover:text-gray-900">
              {n.label}
            </a>
          ))}
          <div className="flex items-center gap-2">
            <button className={`btn ${lang === "ru" ? "btn-primary" : "btn-outline"}`} onClick={() => setLang("ru")}>
              RU
            </button>
            <button className={`btn ${lang === "en" ? "btn-primary" : "btn-outline"}`} onClick={() => setLang("en")}>
              EN
            </button>
          </div>
        </nav>
          ...
          <div>
            <figure className="relative aspect-[16/10] w-full rounded-3xl shadow-xl overflow-hidden">
              ...
              <figcaption className="absolute bottom-0 left-0 right-0 bg-black/40 text-white text-xs p-2">
                Ми-8: финальная приёмка на площадке
              </figcaption>
            </figure>
          </div>
        </div>
    ...
    <Section id="experience" className="bg-gray-50">
      <Container>
        <div className="max-w-3xl">
          ...
          <ul className="mt-4 grid gap-3 text-gray-800">
            {t.expFacts.map((f: string) => (
              <li key={f}>• {f}</li>
            ))}
          </ul>
        </div>
...
/** ====== Блог ====== */
function BlogPosts({ t, lang }: { t: any; lang: Locale }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const { data, usedLocale: fetchedLocale } = await fetchPostsWithFallback(lang);
        if (cancelled) return;
        setPosts(data);
  ...
  const fmtDate = (iso?: string) => {
    ...
    try {
      ...
      return d.toLocaleDateString(usedLocale === "ru" ? "ru-RU" : "en-US", {
        year: "numeric",
        month: "short",
        day: "2-digit",
      });
  ...
  return (
    <Section id="blog">
      <Container>
        <div className="max-w-3xl">
          <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">{t.blogTitle}</h2>
          <p className="mt-3 text-gray-700">{t.blogLead}</p>
        </div>

        {loading && <p className="mt-6 text-gray-500">Загрузка…</p>}
        {!loading && posts.length === 0 && (
          <p className="mt-6 text-gray-600">
            {usedLocale === "ru" ? "Постов пока нет." : "No posts yet."}
          </p>
        )}
        ...
        <div className="mt-8 grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {posts.slice(0, 6).map((p: any) => {
            ...
            const date = fmtDate(a.publishedAt ?? a.createdAt);
            return (
              <div key={p.id} className="card overflow-hidden">
                <div className="relative">
                  {cover ? (
                    <img src={mediaUrl(cover)} alt={title} className="w-full h-44 object-cover" />
                  ) : (
                ...
                <div className="card-body">
                  ...
                  {excerpt && <p className="mt-2 text-gray-700 line-clamp-3">{excerpt}</p>}
        ...
        {posts.length > 0 && usedLocale !== lang && (
          <p className="mt-6 text-xs text-gray-500">
            {lang === "ru"
              ? "Пока нет русской версии постов — показаны английские."
        ...
        <div className="mt-8 grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((h: any) => {
            ...
            const currency: Currency =
              a.currency === "RUB" || a.currency === "USD" ? a.currency : "USD";

            const price =
              typeof rawPrice === "number"
                ? formatPrice(rawPrice, currency, usedLocale)
                : usedLocale === "ru"
                ? "Цена по запросу"
                : "Price on request";
        ...
        <div className="grid lg:grid-cols-2 gap-8 items-start">
          <div>
            ...
            <div className="mt-6 space-y-2 text-gray-100">
              <a className="flex items-center gap-2" href={`tel:${CONTACT.phoneHref}`}>
                {CONTACT.phoneDisplay}
              </a>
              <a className="flex items-center gap-2" href={`https://t.me/${CONTACT.telegram}`} target="_blank">
                {"@" + CONTACT.telegram}
              </a>
          ...
          <div className="card bg-white text-gray-900">
            ...
            <div className="card-body">
              <form className="space-y-3" onSubmit={(e) => e.preventDefault()}>
                <div>
                  <label className="text-sm text-gray-700">Имя / Name</label>
                  <input
                ...
                <div>
                  <label className="text-sm text-gray-700">Телефон / Phone</label>
                  <input
                    className="w-full border rounded-xl px-3 py-2"
                    value={phone}
                ...
                <div>
                  <label className="text-sm text-gray-700">Сообщение / Message</label>
                  <textarea
                    className="w-full border rounded-xl px-3 py-2"
                    rows={4}
                    value={message}
                    onChange={(e) => setMessage(e.target.value)}
                    placeholder="Кратко опишите задачу / Describe your mission"
                  />
                </div>
                <div className="flex gap-3 pt-2">
                  ...
                  <a className="btn btn-outline" href={`https://t.me/${CONTACT.telegram}`} target="_blank">
                    Написать в Telegram
                  </a>
                </div>
              </form>
    ...
    <footer className="border-t border-gray-100">
      <Container className="py-8 text-sm text-gray-600 flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-4">
          <a href={`tel:${CONTACT.phoneHref}`} className="flex items-center gap-1">
            {CONTACT.phoneDisplay}
          </a>
          <a href={`mailto:${CONTACT.email}`} className="flex items-center gap-1">
            Mail
          </a>
          <a href={`https://t.me/${CONTACT.telegram}`} className="flex items-center gap-1">
            Telegram
          </a>
        </div>
      </Container>
